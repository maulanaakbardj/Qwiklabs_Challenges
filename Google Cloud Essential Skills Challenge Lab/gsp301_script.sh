...
gsutil cp gs://sureskills-ql/challenge-labs/ch01-startup-script/install-web.sh gs://<GCP_Project_ID>
...
gcloud compute instances create example-instance --zone=us-central1-a --tags=http-server --metadata startup-script-url=gs://<GCP_Project_ID>/install-web.sh
...
# After firewall creation (Task 3) just wait and then check the scor
